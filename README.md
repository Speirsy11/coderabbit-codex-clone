...
For `all` and `uncommitted` reviews, small untracked text files are included in the review input. Large, binary, unreadable, and non-file untracked paths are skipped and reported in JSONL context/warning events. Default path filters exclude generated/dependency/media artifacts such as `node_modules/**`, `dist/**`, lockfiles, minified bundles, source maps, and common binary extensions; add repo-specific filters with `pathFilters` in `crx.config.json`. `pathInstructions` can attach glob-scoped guidance to matching changed files, and common guideline files such as `AGENTS.md`, `CLAUDE.md`, `GEMINI.md`, `.cursorrules`, and `.github/copilot-instructions.md` are auto-loaded when present. Directory-scoped guideline filenames such as `AGENTS.md` are also discovered along changed-file paths, so `src/AGENTS.md` can guide reviews that touch `src/**` without affecting unrelated directories. Very large guideline and local-tool prompt sections are compacted with explicit truncation markers before review. If the diff exceeds `--max-diff-bytes`, it is truncated and the truncation is reported in plain and JSONL output. Large instruction-file and local-tool contexts are also compacted before prompt construction with explicit `CRX_*_TRUNCATED` markers so review prompts stay bounded.
