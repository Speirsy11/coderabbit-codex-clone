...
- **Implemented scope/config subset:** path filters with excluded-file stats, path instructions, common guideline auto-loading with directory-scoped ancestor lookup, custom guideline patterns, `chill`/`assertive` review profiles, presets for Node/Python/Ruby, and safe `.coderabbit.yaml`/`.coderabbit.yml` fallback mapping.
...
| Path controls | Path filters exclude generated, lock, binary, media files | Path docs describe filters and defaults. <https://docs.coderabbit.ai/configuration/path-instructions.md> | P0 | Implemented local subset | Local filtering runs before prompt construction, defaults cover common generated/lock/binary/media paths, and JSONL metrics report excluded-file stats for auditability. |
